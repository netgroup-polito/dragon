...
# files to fill with overall results
OUTPUT_CONVERGENCE = 'output_times.dat'
OUTPUT_ITERATIONS = 'output_iterations.dat'
OUTPUT_MESSAGES = 'output_messages.dat'
OUTPUT_CONCURRENCY = 'output_concurrency.dat'
...
class InvalidIteration(Exception):
    def __init__(self, message):
        ...
        super(InvalidIteration, self).__init__(message)
...
def extend_output_file(file_name, new_values, n_nodes):
    rows = []
    if not os.path.exists(file_name):
        with open(file_name, 'w') as o_file:
            o_file.write("# 50sdos 100sdos 150sdos 200sdos 250sdos 300sdos\n\n")
    ...
    with open(file_name, "r") as o_file:
        ...
        # jump to proper slice
        for row in reader:
            rows.append(row)
            if ' '.join(row) == "# {} nodes".format(n_nodes):
                new = False
                break
        ...
        if not new:
            ...
            for row in reader:
                ...
                if not row:
                    break
                value = new_values.pop(0)
                element = "{0:.3f}".format(value) if type(value) == float else str(value)
                row.append(element)
            ...
            # rows.append([])
            rows.append([])
            rows.append("# {} nodes".format(n_nodes).split(' '))
...
if len(sys.argv) != 4:
    print("usage python3 -m tests.test_multi_run <sdos_n> <nodes_n> <iterations>")
    exit(1)
...
while iteration < iterations:
        ...
        if len(rap.sdos) > 0:
            ...
            for sdo in rap.sdos:
                if sdo in to_deploy:
                    ...
                    raise InvalidIteration("wrong sdo in rap problem")
            ...
            if killed:
                raise InvalidIteration("there is at least an sdo killed for timeout")
            ...
            raise InvalidIteration("no sdo decided to ask for new resources")
        ...
        if len(rap.sdos) > 0:
            for sdo in rap.sdos:
                ...
                if sdo_name in killed:
                    ...
                    last_update_times.append(-1)
                    agreement_times[sdo_name] = -1
                    placements[sdo_name] = []
                ...
                try:
                    with open(results_file) as f:
                        ...
                        last_update_times.append(results["last-update"] if len(rap.sdos) > 1 else 0.010)
                        agent_iterations.append(results["iterations"])
                        agreement_times[sdo_name] = results["agreement"] if len(rap.sdos) > 1 else 0.010
            ...
            if any(agreement_times[sdo] < 0 for sdo in agreement_times):
                raise InvalidIteration("there is at least a weak agreement")
            ...
            convergence_times.append(max(last_update_times))
            convergence_messages.append(sum(list(sent_messages.values())))
            concurrency.append(len(rap.sdos))
            dragon_iterations.append(max(agent_iterations))

    except InvalidIteration as err:
        print("WARNING: {}, skipping this iteration.".format(err.message))
        winners = []
        to_deploy = []
        to_update = []
        iteration -= 1
    ...
    for sdo in sdos:
        if sdo in to_delete:
            ...
            if sdo in to_deploy:
                delete_probability[sdo] = unit*random.randrange(2)
                deploy_probability[sdo] = 0
                update_probability[sdo] = 0
            elif sdo in to_update:
                deploy_probability[sdo] = 0
            ...
            delete_probability[sdo] += unit*random.randint(-1, 1)
            deploy_probability[sdo] = 0
            update_probability[sdo] += unit*random.randint(-1, 1)
        else:
            delete_probability[sdo] = 0
            deploy_probability[sdo] += unit*random.randint(-1, 1)
            update_probability[sdo] = 0
...
# [ add results to file ]
extend_output_file(OUTPUT_CONVERGENCE, convergence_times, nodes_number)
# extend_output_file(OUTPUT_UPDATES, last_update_times, nodes_number)
extend_output_file(OUTPUT_MESSAGES, convergence_messages, nodes_number)
extend_output_file(OUTPUT_ITERATIONS, dragon_iterations, nodes_number)
extend_output_file(OUTPUT_CONCURRENCY, concurrency, nodes_number)
